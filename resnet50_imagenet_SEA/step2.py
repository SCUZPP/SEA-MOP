...
def evaIndividual(ind, model, args, train_loader, test_loader, val_loader, original_filter):
    ...
    #model_new = torch.nn.DataParallel(model_new).cuda()    
    #计算微调后的准确率
    ...
    #打印微调前的准确率和损失
    print('step2:  * accuracy {acc:.2f}, pruning {pruning:.2f}'
...
def initialization(pop_size, gene_length, model, args, train_loader, test_loader, val_loader, original_filter, p0, target_dir):
    ...
    for i in range(pop_size):
        ...
        if args.retrain:
            ...
            with open(path_save + "population_300_0.pkl", 'wb') as f:
                pickle.dump(population, f) 
                
            with open(path_save + "population_60_0.pkl", 'wb') as f:
                pickle.dump(acc, f) 
...
def get_population(model, args, train_loader, test_loader, val_loader):
    ...
    #p0 = []
    original_filter = pickle.load(open('filter_dict/filter_dict_resnet%s.pkl' % (args.depth),  'rb'))
    p0 = pickle.load(open('population_dict/population_dict_resnet%s_16_p0.pkl' % (args.depth), 'rb'))
    #p0.append(p[0])
