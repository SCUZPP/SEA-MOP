...
def evaIndividual(ind, model, args, train_loader, test_loader, val_loader, original_filter):
    ...
    print('step1:  * accuracy {acc:.2f}, loss {loss:.2f}, pruning {pruning:.2f}'
...
def get_population(model, args, train_loader, test_loader, val_loader):
    ...
    target_dir = 'Results_resnet%s/' % (args.depth)
    ...
    #以文件形式存储
    pickle.dump(population_init, open('population_dict/population_dict_resnet%s.pkl' % (args.depth), 'wb'))
