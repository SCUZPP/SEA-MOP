...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, fp, cfg, expansion=1, growthRate=12, dropRate=0):
    ...
    def forward(self, x):
        ...
        #print('bbn', out.shape)
        out = self.select(out)
        #print('bselect', out.shape)
        out = self.relu(out)
        out = self.conv1(out)
        
        #print('bconv', out.shape)
        if self.dropRate > 0:
...
class Transition(nn.Module):
    def __init__(self, inplanes, fp, outplanes, cfg):
...
class densenet(nn.Module):
    ...
    def _make_denseblock(self, block, blocks, cfg):
        ...
        for i in range(blocks):
            ...
            layers.append(block(self.inplanes, self.fp, cfg = cfg[i], growthRate=self.growthRate, dropRate=self.dropRate))
    ...
    def _make_transition(self, compressionRate, cfg):
        ...
        return Transition(inplanes, self.fp, outplanes, cfg)
