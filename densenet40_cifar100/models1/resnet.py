...
class BasicBlock(nn.Module):
    expansion = 1
    def __init__(self, fp, inplanes, planes, cfg, stride=1, downsample=None):
    ...
    def forward(self, x):
        ...
        #self.fp.append(out)
        self.fp.append(out)
        #print('self.fp.append(out)', len(self.fp))
        out = self.conv2(out)
        out = self.bn2(out)
        self.fp.append(out)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, cfg, stride=1):
        ...
        layers = []
        layers.append(block(self.fp, self.inplanes, planes, cfg[0], stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.fp, self.inplanes, planes, cfg[i]))
    ...
    def forward(self, x):
        ...
        x = self.relu(x)    # 32x32
        self.fp.append(x)
        #print('self.fp.append(x)', len(self.fp))
        x = self.layer1(x)  # 32x32 
        
        #print('self.layer1(x)', len(self.fp))
        x = self.layer2(x)  # 16x16
        
        #print('self.layer2(x)', len(self.fp))
        x = self.layer3(x)  # 8x8
        
        #print('self.layer3(x)', len(self.fp))
